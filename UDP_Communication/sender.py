...
sender.py
---------
Broadcasts an array of 100 sixteen-bit integers to every computer on the
local network using a SINGLE UDP datagram.

How the four requirements are met:
  * Only the standard `socket` module is used to send the data.
  * The whole array leaves in ONE transmission (UDP broadcast).
  * Long runs of the same value are compressed (run-length encoding).
  * The data is validated before sending, and the receiver can re-validate it.

Testing:
  Leave TARGET_IP = "127.0.0.1" to test against a local receiver.
  For the real 20-machine network set TARGET_IP = "192.168.1.255"
  (the subnet's broadcast address) so 192.168.1.1 .. .20 all receive it
  from the one sendto() call.
"""
...
def main():
    ...
    # Allow sending to a broadcast address. Harmless when TARGET_IP is a
    # plain unicast/localhost address, required when it is x.x.x.255.
